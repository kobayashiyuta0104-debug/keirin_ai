# -*- coding: utf-8 -*-
"""
競輪AI：配当クラス±1購入シミュレーション

【目的】
AIが予想した「配当クラス」と、その前後1クラスに入っている3連単を全て100円ずつ購入した場合の
購入額・払戻・収支・回収率を調査する。
...
1レースにつき、
「AI予想クラス」と「前後1クラス」の範囲に入る3連単を全て100円購入する。

例：
AI予想が「10,000～19,999円」の場合、
「5,000～9,999円」「10,000～19,999円」「20,000～29,999円」
に該当する3連単を全て100円購入する。
...
OUTPUT_DIR = Path(
    r"C:\競輪AI\csv\analysis\018_simulate_ai_purchase"
)
...
def get_plus_minus_one_class_range(ai_class):
    """
    AI予想クラスを中心に、前後1クラスを購入対象にする。
    ...
    端のクラス：
        0～1,999円       → 自クラス + 次クラス
        100,000円以上    → 前クラス + 自クラス

    隣接クラスのオッズ範囲は連続しているため、
    実際の購入判定では前後クラスを含む一つの範囲として扱う。
    """
    cls = str(ai_class).strip()
    ...
    end_idx = min(len(CLASS_ORDER) - 1, idx + 1)

    low = CLASS_RANGE[CLASS_ORDER[start_idx]][0]
    high = CLASS_RANGE[CLASS_ORDER[end_idx]][1]
...
def main():
    ...
    for idx, row in df.iterrows():
        ai_class = row["AI予想"]
        # AI予測クラス ±1クラスを購入対象にする
        # 例：10,000～19,999円予測
        #     → 5,000～9,999円
        #     → 10,000～19,999円
        #     → 20,000～29,999円
        class_range = get_plus_minus_one_class_range(ai_class)
    ...
    output_excel = OUTPUT_DIR / "018_simulate_ai_purchase_result.xlsx"
