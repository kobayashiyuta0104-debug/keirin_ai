...
OUTPUT_FILE = "183_verified_jsj006_jsj012_capture.json"
...
def main():
    print("=" * 70)
    print("🔥 183 JSJ006 7/7完全一致 → hidden競走結果DOM発火 → JSJ012取得")
    print("=" * 70)
    ...
    with sync_playwright() as p:
        ...
        def handle_response(response):
            if "type=JSJ012" in response.url:
                with state_lock:
                    race_key = state["race_key"]
                    venue = state["venue"]
                    race_no = state["race_no"]
                if not race_key or race_key not in captures:
                    return
                try:
                    data = response.json()
                    request = response.request
                    jsj012_captures[race_key] = {
                        "race_key": race_key,
                        "venue": venue,
                        "race_no": race_no,
                    ...
                    print("🏁🏁🏁 JSJ012取得成功:", race_key)
                except Exception as e:
                    print("❌ JSJ012 JSON解析/保存失敗:", e)
                return
        ...
        for index, target in enumerate(race_map, start=1):
            ...
            if success:
                ...
                result_button = page.locator("#pibtnResult")
                try:
                    if result_button.count() > 0:
                        print("🔥 競走結果CLICK:", race_key)
                        page.evaluate(
                            """
                            () => {
                                const btn = document.querySelector("#pibtnResult");
                                if (!btn) {
                                    throw new Error("pibtnResult not found");
                                }
                                btn.click();
                            }
                            """
                        ...
                        for _ in range(40):
                            page.wait_for_timeout(500)
                            if race_key in jsj012_captures:
                                break
                        if race_key in jsj012_captures:
                            print("✅ JSJ012取得完了:", race_key)
                        else:
                            print("❌ JSJ012受信なし:", race_key)
                    else:
                        print("❌ pibtnResultなし:", race_key)
                except Exception as e:
                    print("❌ 競走結果CLICK失敗:", e)
        ...
        output = {
            ...
            "jsj012_capture_count": len(jsj012_captures),
            "jsj012_captures": [
                jsj012_captures[x["race_key"]]
                for x in race_map
                if x["race_key"] in jsj012_captures
            ],
            "missing": missing,
        ...
        print("🏁 JSJ012取得数:", len(jsj012_captures))
