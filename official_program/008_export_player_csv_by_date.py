...
def main():
    print("===" * 20)
    print("008 Player CSV Exporter By Date")
    print("===" * 20)
    ...
    while current <= end:
        ...
        pre_race_path = DAILY_DIR / f"{target_date}_pre_race.json"

        print()
        print("=" * 60)
        print(f"対象日付: {target_date}")

        if not pre_race_path.exists():
            print("pre_race.json が見つからないためスキップ")
            current += timedelta(days=1)
            continue

        print(f"読込: {pre_race_path.name}")
        ...
        output_path = save_player_csv(rows, target_date)
